...
<script lang="ts">
  ...
  function toggle(storm: Storm) {
    StormData.show = true;
    StormData.storm = storm;
  }

  function getIconSize(storm: Storm) {
...
{#each $storms as storm}
  {#if storm.id}
    <div
      id={"storm-" + storm.id + get(current_datetime).ts}
      style={storm.settings.visible
        ? "visibility:visible"
        : "visibility:hidden"}
    >
      <button
        class="storm"
        id={"btn-" + storm.id + get(current_datetime).ts}
      ...
      {:else}
        <Tooltip
          target={"btn-" + storm.id + get(current_datetime).ts}
          placement="top">{storm.id}</Tooltip
        >
      {/if}
    </div>
  {/if}
