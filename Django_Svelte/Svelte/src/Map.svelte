    ...
    function createMap() {
        ...
        const mousePositionControl = new MousePosition({
            coordinateFormat: function createStringXY(coordinate) {
                ...
                return (
                    Math.abs(coordinate[1]).toFixed(4) +
                    "&deg" +
                    lat_sign +
                    " " +
...
<div style="visibility: {fetchingInitialData?'hidden':'visible'};">
    <div id="map" class="map" />
    <div id="mouse-position" />

    <!-- Mostrar las celdas de tormenta -->
    <StormOL bind:StormData {show_label} />
    ...
    <StormModal bind:StormData />

    <!-- Mostrar la tabla de las celdas de tormenta -->
    <StormTable bind:showStormTable bind:show_label />
