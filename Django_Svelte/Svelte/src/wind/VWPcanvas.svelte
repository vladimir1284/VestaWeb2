<script>
  ...
  function drawColumn(ctx, vwp, x0, y0, hdelta){
      ...
      for (let i = 0; i < hdata.length; i++){
        drawBarb(ctx, vwp.data[i], x0, y0 - (2 + i)*hdelta, hdelta*1.4)
      }
  }

  function prepareHeights(ctx, l, h, x0, y0){
    let hspace = 10
    if (vwp[0].data.length > 0){
      hdata = []
      hdata = parseVWP(vwp[0].data)
      // Vertical spacing
      hspace = h/(hdata.length + 3)
      // Draw horizontal lines
      ctx.beginPath()
      ctx.strokeStyle = "#D0D0D0"
      hdata.forEach((element, i) => {
        const y = y0 - (2 + i)*hspace
        ctx.moveTo(x0, y)
        ctx.lineTo(x0+l, y)
        ctx.fillText(element.ht, x0 - 32, y + 5)
      })
      ctx.stroke()
    }
    return hspace
