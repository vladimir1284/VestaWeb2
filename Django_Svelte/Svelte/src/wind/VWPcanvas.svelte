<script lang="ts">
  ...
  import type { VWPelement } from "../backend";
  import { _ } from "../services/i18n";
  import { DateTime } from "luxon";

  const alturas = [
    0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5, 5.5, 6, 8, 10, 12, 14,
  ]; // km
  ...
  function ft2km(value: number): number {
    return (value * 0.3048) / 1000;
  }

  function ft2kmSTR(value: number): string {
    let str_value = "0";
  ...
  function drawBarbs() {
    var canvas = <HTMLCanvasElement>document.getElementById("vwp_barbs");
    if (canvas.getContext) {
      ...
      for (let i = 0; i < nvalues; i++) {
        const data = { vel: (i * 5) / 1.943844, rms: -1 };
  ...
  function drawColumn(
    ...
    ctx.fillStyle = "black";
    ctx.fillText(
      DateTime.fromISO(vwp.datetime).setZone("local").toFormat("HH:mm"),
      x0 - 12,
      y0 + 15
    );
